...
- 批次创建时自动签发一条凭证。
- 标签打印接口读取标签载荷时会签发一条新的可打印凭证并返回明文二维码；旧的未吊销凭证继续有效。
...
| `scanner_user` | `varchar(255)` | 是 | 无 | - | 扫码用户标识，当前无认证时为空 |
...
CREATE INDEX IF NOT EXISTS batch_qr_credentials_batch_revoked_idx
...
CREATE INDEX IF NOT EXISTS qr_scan_audit_logs_batch_scanned_idx
    ON qr_scan_audit_logs(batch_id, scanned_at_server DESC);
CREATE INDEX IF NOT EXISTS qr_scan_audit_logs_client_scan_idx
